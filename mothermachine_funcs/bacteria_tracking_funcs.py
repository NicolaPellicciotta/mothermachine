...
#            
#           here I want to label the bacteria only the bac taken in consideration 
            newwell, bacteria_lineage_new = label_most_likely(
                best_option, new_well, bacteria_lineage, important_bac=important_bac)
        check_probs[num] = check_prob
        out_wells[num] = newwell

    return out_wells, bacteria_lineage_new, check_probs
...
def find_changes(in_list, option_list, well, new_well):
    ...
    for jj in ind_centroids:
        for i, region in enumerate(regionprops(well)):
            if i==jj:
                #measurements_in[ind_cc] = [region.centroid[0]+shift_centroid*region.centroid[0], region.area,region.major_axis_length] # i change area and major axis
...
#    for i, region in enumerate(regionprops(well)):  # need to be sorted by centroid
        # find y centr coord and area of old each bac
#        measurements_in[i] = [region.centroid[0]+shift_centroid*region.centroid[0], region.area,region.major_axis_length]
    for j, region2 in enumerate(regionprops(new_well)):
        # find y centr coord and area of each new bac
        #measurements_out[j] = [region2.centroid[0], region2.area,region.area,region.major_axis_length] 
...
#            in_options_dict[in_options] = [num_divs, area_chan, centr_chan, measurements_in[in_num][0],measurements_in[in_num][2]]
...
def label_all_new(new_well, label_dict_string):
    """
...
def label_most_likely(most_likely, new_well, label_dict_string,important_bac=None):
    ...
    if important_bac is not None:
        jjj=0
        for jj in arange(1,len(most_likely)+1):
            if not len(unique(most_likely[:jj]))> uint(important_bac):
                jjj+=1
        most_likely=most_likely[:jjj]
